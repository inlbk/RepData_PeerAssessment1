---
title: "Analysis of Daily Human Step Activity"
author: "inlbk"
date: "March 28 , 2017"
output: 
  html_document:
...
The histogram above shows that the most frequently occurring daily step total is between 10,000 and 12,000 steps. The mean number of steps taken per day is `r round(mean(stepsbydate$steps),digits=2)` and the median number of steps taken per day is `r round(median(stepsbydate$steps),digits=2)`, as indicated by the red dashed line in the histogram.
...
An analysis of the data reveals that there are 11,014 observations (during the 53 days in which data was recorded) in which the recorded number of steps during a 5-minute interval is 0.  This is the equivalent of about `r round(length(which((rawsteps[,1]==0)))/53./12.,digits=1)` hours per day with zero steps. This seems highly sedentary, but without knowing more about the actual activity level of the subject, no further analysis can be performed to determine its accuracy.
